...
figure; plot(X,N,'ko-'); title('Histogram of sample interval for sci-m-present-time>0, before sorting');

% Look at ctd time
figure; plot(ctd_time,'bo-'); title('Sample time for ctd'); xlabel('Sample Number'); ylabel('Secs since 1/1/1970');
ctd_timegt0 = ctd_time(ctd_time>0);
plot0 = zeros(1,length(ctd_timegt0));
figure; plot(ctd_timegt0,plot0,'ro-'); title('Sample time>0 for ctd, before sorting'); xlabel('Secs since 1/1/1970');
figure; plot(diff(ctd_timegt0),'bo-'); title('Sample interval for ctd>0, before sorting'); xlabel('Sample Number'); ylabel('Secs');
[N,X] = hist(diff(ctd_timegt0)); 
figure; plot(X,N,'ko-'); title('Histogram of sample interval for ctd, before sorting');

% Compare time bases
plot0 = zeros(1,length(ptime_ebdgt0));
figure; plot(ptime_ebdgt0,plot0,'bo-'); hold on;
plot0 = zeros(1,length(ctd_timegt0));
plot(ctd_timegt0,plot0,'ro-'); hold on;
plot0 = zeros(1,length(ptime_ebd(ctd_time<=0)));
plot(ptime_ebd(ctd_time<=0),plot0,'k+-'); 
legend('sci','ctd>0','sci when ctd=0'); xlabel('Secs since 1/1/1970');
...
[N2_temp,X2_temp] = hist(temp);
[N2_cond,X2_cond] = hist(cond);
